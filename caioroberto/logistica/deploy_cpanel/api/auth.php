...
switch ($action) {
    ...
    case 'request-reset':
        ...
        // Buscar nome do usuário para personalizar o e-mail
        $stmtName = $pdo->prepare("SELECT name FROM users WHERE id = ?");
        $stmtName->execute([$user['id']]);
        $userName = $stmtName->fetchColumn() ?: '';

        $mailSubject = 'Código de Recuperação - NTO Logística';
        $mailBody    = "Olá, {$userName}!\n\n"
                     . "Você solicitou a recuperação de senha no sistema NTO Logística.\n\n"
                     . "Seu código de recuperação é:\n\n"
                     . "    {$code}\n\n"
                     . "Este código é válido por 1 hora.\n\n"
                     . "Se você não solicitou a recuperação de senha, ignore este e-mail.\n\n"
                     . "-- NTO Logística Medclin";

        $emailSent = sendSmtpMail($data['email'], $userName, $mailSubject, $mailBody);

        $message = $emailSent
            ? 'Código enviado para seu e-mail! Verifique sua caixa de entrada (e spam).'
            : 'Código gerado. Contate o administrador para obtê-lo.';
